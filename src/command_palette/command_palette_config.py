...
class EditSettings:
    def __init__(self) -> None:
        with path.default_settings.open("r") as config_file:
            self.settings_data = yaml.safe_load(config_file)


class DisplaySettings(EditSettings):
    def __init__(self) -> None:
        super().__init__()
    ...
    def display_settings(self):
        ...
        selected_key = list(self.settings_data)[
    ...
    def _retrieve_formatted_settings(self):
        keys = list(self.settings_data)
        values = [
            value if not isinstance(value, dict) else ""
            for value in self.settings_data.values()
        ]
        return keys, values
...
class UpdateSpecifiedSettings(EditSettings):
    def __init__(self, setting_to_edit) -> None:
        super().__init__()
        self.user_edit = None
        self.setting_to_edit = setting_to_edit
        self.previous_setting = self.settings_data[self.setting_to_edit]
    ...
    def update_specified_setting_through_user(self):
        ...
        with path.default_settings.open("w") as config_file:
            self.settings_data[self.setting_to_edit] = (
                self.user_edit if self.user_edit else self.previous_setting
            )

            yaml.dump(
                self.settings_data, config_file, default_flow_style=False, indent=4
            )
...
# UpdateSpecifiedSettings("lyrics_enabled").update_specified_setting()
