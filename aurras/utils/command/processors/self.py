...
logger = get_logger("aurras.command.processors.self")
...
class SelfProcessor:
    ...
    def get_version_info(self) -> int:
        ...
        with logger.operation_context("self_get_version_info"):
            try:
                ...
                logger.error(
                    "Failed to get version information", error=str(e), exc_info=True
                )
                return 1
    ...
    def uninstall(self) -> int:
        ...
        """
        with logger.operation_context("self_uninstall"):
            if self.is_development:
                console.print_info(
                    "Development installation - just delete the directory"
                )
                console.print_info(f"Directory: {PROJECT_ROOT}")
                logger.info("Uninstall requested for development installation")
                return 0

            remove_data = console.confirm(
                "Remove all Aurras data as well?", default=False
            )
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.info("Aurras uninstalled successfully via pip")
                else:
                    ...
                    logger.warning(
                        "Pip uninstallation failed", return_code=result.returncode
                    )
                    return 1

            except Exception as e:
                console.print_error(f"Uninstallation failed: {e}")
                logger.error("Uninstallation failed", error=str(e), exc_info=True)
                return 1
            ...
            if remove_data:
                try:
                    ...
                    logger.info(
                        "Removed Aurras data directory",
                        app_dir=str(_path_manager.app_dir),
                    )
    ...
    def check_dependencies(self) -> int:
        ...
        Returns:
            int: Exit code (0 for success, 1 for error)
        """
        with logger.operation_context("self_check_dependencies"):
            pyproject_file = PROJECT_ROOT / "pyproject.toml"

            if not pyproject_file.exists():
                console.print_error("pyproject.toml not found!")
                logger.error("pyproject.toml not found", project_root=str(PROJECT_ROOT))
                return 1
            ...
            try:
                ...
                logger.error(
                    "Failed to parse pyproject.toml", error=str(e), exc_info=True
                )
                return 1
