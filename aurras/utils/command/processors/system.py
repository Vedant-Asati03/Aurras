...
logger = get_logger("aurras.command.processors.system")
...
class SystemProcessor:
    ...
    def show_cache_info(self) -> int:
        ...
        """
        with logger.operation_context(operation="system_show_cache_info"):
            if _path_manager.cache_db.exists():
                ...
                logger.warning("Cache database not found")
                return 1
    ...
    def cleanup_cache(self, days: Optional[int] = 30) -> int:
        ...
        Returns:
            int: Exit code (0 for success, 1 for error)
        """
        with logger.operation_context(operation="system_cleanup_cache", days=days):
            ...
            try:
                ...
                if _path_manager.cache_db.exists():
                    ...
                    logger.warning("Cache database not found during cleanup")
                ...
                if sum(results.values()) > 0:
                    ...
                    logger.info(
                        ...
                        searches_deleted=results["searches"],
                        lyrics_deleted=results["lyrics"],
                    ...
                    logger.info("Cache was already clean", days_to_keep=days_to_keep)

                return 0

            except Exception as e:
                console.print_error(f"Error cleaning cache: {str(e)}")
                logger.error(f"Error in cleanup_cache: {str(e)}", extra={"error_type": type(e).__name__, "error_message": str(e)})
                return 1
