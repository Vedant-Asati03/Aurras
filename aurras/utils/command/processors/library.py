...
logger = get_logger("aurras.command.processors.library")
...
class LibraryProcessor:
    ...
    def search(self, query: str, limit: int = 5) -> int:
        ...
        with logger.operation_context(
            operation="library_search", query=query, limit=limit
        ):
            if not query:
    ...
    def list_downloads(self) -> int:
        ...
        Returns:
            int: Exit code (0 for success, 1 for error)
        """
        with logger.operation_context(operation="library_list_downloads"):
            try:
                downloads = self.downloader.list_downloads()

                if not downloads:
                    console.print_info("No downloaded songs found")
                    logger.info("No downloaded songs found")
                    return 0
                ...
                console.print(table)
                logger.info("Listed downloaded songs", download_count=len(downloads))
                return 0
