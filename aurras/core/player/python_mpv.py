...
if os.name == "nt":
    ...
    try:
        ...
        if not os.path.isabs(
            dll
        ):  # can only be find_library, not the "look next to mpv.py" thing
            raise OSError(
            ...
            raise OSError(
                f"ctypes.find_library found mpv.dll at {dll}, but ctypes.CDLL could not load it."
            ) from e
    fs_enc = "utf-8"
...
class ErrorCode(object):
    ...
    EXCEPTION_DICT = {
        ...
        -11: lambda *a: RuntimeError(
            "Generic error getting or setting mpv property", *a
        ),
...
class MpvEventID(c_int):
    ...
    def __repr__(self):
        return (
            f"<MpvEventID {self.value} {_mpv_event_name(self.value).decode('utf-8')}>"
        )
...
class MpvNodeList(Structure):
    ...
    def dict_value(self, decoder=identity_decoder):
        return {
            self.keys[i].decode("utf-8"): self.values[i].node_value(decoder)
...
class MpvEventProperty(Structure):
    ...
    @property
    def value(self):
        return MpvNode.node_cast_value(
            self.data, self.format.value, decoder=lazy_decoder
        )
    ...
    if deprecated:
        ...
        def wrapper(*args, **kwargs):
            if not wrapper.warned:  # Only warn on first invocation to prevent spamming
                warn(
                    "Backend C api has been deprecated: " + name,
                    DeprecationWarning,
                    stacklevel=2,
                )
                wrapper.warned = True
...
_handle_func(
    "mpv_command_node", [POINTER(MpvNode), POINTER(MpvNode)], c_int, ec_errcheck
)
_handle_func(
    "mpv_command_node_async", [c_ulonglong, POINTER(MpvNode)], c_int, ec_errcheck
)
...
_handle_func(
    ...
    [c_ulonglong, c_char_p, MpvFormat, c_void_p],
    c_int,
    ec_errcheck,
)
...
_handle_func(
    "mpv_get_property_async", [c_ulonglong, c_char_p, MpvFormat], c_int, ec_errcheck
)
_handle_func(
    "mpv_observe_property", [c_ulonglong, c_char_p, MpvFormat], c_int, ec_errcheck
)
...
_handle_func(
    "mpv_event_to_node",
    [POINTER(MpvNode), POINTER(MpvEvent)],
    c_int,
    ec_errcheck,
    ctx=None,
)
...
_handle_func(
    "mpv_stream_cb_add_ro", [c_char_p, c_void_p, StreamOpenFn], c_int, ec_errcheck
)

_handle_func(
    "mpv_render_context_create",
    [MpvRenderCtxHandle, MpvHandle, POINTER(MpvRenderParam)],
    c_int,
    ec_errcheck,
    ctx=None,
)
...
_handle_func(
    ...
    [RenderUpdateFn, c_void_p],
    None,
    errcheck=None,
    ctx=MpvRenderCtxHandle,
)
_handle_func(
    "mpv_render_context_update", [], c_int64, errcheck=None, ctx=MpvRenderCtxHandle
)
_handle_func(
    "mpv_render_context_render",
    [POINTER(MpvRenderParam)],
    c_int,
    ec_errcheck,
    ctx=MpvRenderCtxHandle,
)
_handle_func(
    "mpv_render_context_report_swap", [], None, errcheck=None, ctx=MpvRenderCtxHandle
)
...
def _make_node_str_list(l):
    ...
    node_list = MpvNodeList(
        num=len(l),
        keys=None,
        values=(MpvNode * len(l))(
            *[
                MpvNode(format=MpvFormat.STRING, val=MpvNodeUnion(string=p))
                for p in char_ps
            ]
        ),
    )
    node = MpvNode(
        format=MpvFormat.NODE_ARRAY, val=MpvNodeUnion(list=pointer(node_list))
    )
...
def _make_node_str_map(d):
    ...
    char_ps = [
        (c_char_p(k.encode("utf-8")), c_char_p(_mpv_coax_proptype(v, str)))
        for k, v in d.items()
    ]
    node_list = MpvNodeList(
        num=len(d),
        keys=(c_char_p * len(d))(*[k for k, v in char_ps]),
        values=(MpvNode * len(d))(
            *[
                MpvNode(format=MpvFormat.STRING, val=MpvNodeUnion(string=v))
                for k, v in char_ps
            ]
        ),
    )
...
def _create_null_term_cmd_arg_array(name, args):
    args = (
        [name.encode("utf-8")]
        + [
            (arg if type(arg) is bytes else str(arg).encode("utf-8"))
            for arg in args
            if arg is not None
        ]
        + [None]
...
class _PropertyProxy(_Proxy):
    def __dir__(self):
        return super().__dir__() + [
            name.replace("-", "_") for name in self.mpv.property_list
        ]
...
class FileOverlay:
    def __init__(
        self, m, overlay_id, filename=None, size=None, stride=None, pos=(0, 0)
    ):
        self.m = m
        self.overlay_id = overlay_id
        self.pos = pos
    ...
    def update(self, filename=None, size=None, stride=None, pos=None):
        ...
        self.m.overlay_add(
            self, self.overlay_id, x, y, self.filename, 0, "bgra", w, h, stride
        )
...
class MPV(object):
        ...
        **extra_mpv_opts,
        ...
        if start_event_thread:
            self._event_thread = threading.Thread(
                target=self._loop, name="MPVEventHandlerThread"
            )
    ...
    def _enqueue_exceptions(self):
        try:
            ...
            for fut in self._exception_futures:
                ...
                warn(
                    f"Unhandled exception on python-mpv event loop: {e}\n{traceback.format_exc()}",
                    RuntimeWarning,
                )
            ...
            try:
                ...
                if eid == MpvEventID.COMMAND_REPLY:
                    ...
                    if callback:
                        with self._enqueue_exceptions():
                            callback(
                                ErrorCode.exception_for_ec(event.error), event.data
                            )
                ...
                warn(
                    f"Unhandled {e} inside python-mpv event loop!\n{traceback.format_exc()}",
                    RuntimeWarning,
                )
    ...
    def wait_for_property(
        ...
        """
        with self.prepare_and_wait_for_property(
            name, cond, level_sensitive, timeout=timeout, catch_errors=catch_errors
        ) as result:
            pass
    ...
    @contextmanager
    def prepare_and_wait_for_property(
        self,
        name,
        cond=lambda val: val,
        level_sensitive=True,
    ...
    def wait_for_event(
        self, *event_types, cond=lambda evt: True, timeout=None, catch_errors=True
    ):
        ...
        inside ``cond``.
        """
        with self.prepare_and_wait_for_event(
            *event_types, cond=cond, timeout=timeout, catch_errors=catch_errors
        ) as result:
            pass
        return result.result()
    ...
    def _encode_options(options):
        return ",".join(
            "{}={}".format(_py_to_mpv(str(key)), str(val))
    ...
    def loadfile(self, filename, mode="replace", index=None, **options):
        ...
        if self.mpv_version_tuple >= (0, 38, 0):
            ...
            self.command(
                "loadfile",
                filename.encode(fs_enc),
                mode,
                index,
                MPV._encode_options(options),
            )
        else:
            if index is not None:
                warn(
                    f"The index argument to the loadfile command is only supported on mpv >= 0.38.0"
                )
            self.command(
    ...
    def video_add(self, url, flags="select", title=None, lang=None, albumart=None):
        ...
        self.command(
            "video_add", url.encode(fs_enc), *_drop_nones(flags, title, lang, albumart)
        )
    ...
    def overlay_add(self, overlay_id, x, y, file_or_fd, offset, fmt, w, h, stride):
        ...
        self.command(
            "overlay_add", overlay_id, x, y, file_or_fd, offset, fmt, w, h, stride
        )
    ...
    def message_handler(self, target):
        ...
        def register(handler):
            self._register_message_handler_internal(target, handler)
            handler.unregister_mpv_messages = lambda: self.unregister_message_handler(
                handler
            )
            return handler
    ...
    def event_callback(self, *event_types):
        ...
        def register(callback):
            with self._event_handler_lock:
                self.check_core_alive()
                types = [
                    MpvEventID.from_str(t) if isinstance(t, str) else t
                ...
                wrapper.unregister_mpv_events = partial(
                    self.unregister_event_callback, wrapper
                )
                return wrapper
    ...
    def register_key_binding(self, keydef, callback_or_cmd, mode="force"):
        ...
        if callable(callback_or_cmd):
            ...
            self.register_message_handler(
                "key-binding", self._handle_key_binding_message
            )
            self.command(
                "define-section",
                binding_name,
        ...
        self.command(
            "enable-section", binding_name, "allow-hide-cursor+allow-vo-dragging"
        )
    ...
    def _handle_key_binding_message(
        ...
        *_,
        ...
        self._key_binding_handlers[binding_name](
            key_state, key_name, key_char, scale, arg
        )
    ...
    def register_stream_protocol(self, proto, open_fn=None):
        ...
        def decorator(open_fn):
            ...
            _mpv_stream_cb_add_ro(
                self.handle, proto.encode("utf-8"), c_void_p(), open_backend
            )
    ...
    def _python_stream_open(self, uri):
        ...
        if name in self._python_streams:
            ...
            if self._python_stream_catchall is not None:
                ...
                raise ValueError(
                    "Python stream name not found and no catch-all defined"
                )
    ...
    def python_stream(self, name=None, size=None):
        ...
        def register(cb):
            ...
            if name in self._python_streams:
                raise KeyError(
                    'Python stream name "{}" is already registered'.format(name)
                )
            ...
            def unregister():
                if (
                    name not in self._python_streams
                    or self._python_streams[name][0] is not cb
    ...
    def python_stream_catchall(self, cb):
        ...
        def unregister():
            if self._python_stream_catchall is not cb:
                raise RuntimeError(
                    "This catch-all python stream has already been unregistered"
                )
            self._python_stream_catchall = None
    ...
    def __dir__(self):
        return super().__dir__() + [
            name.replace("-", "_") for name in self.property_list
        ]
...
class MpvRenderContext:
    def __init__(self, mpv, api_type, **kwargs):
        ...
        buf = cast(
            create_string_buffer(sizeof(MpvRenderCtxHandle)),
            POINTER(MpvRenderCtxHandle),
        )
        _mpv_render_context_create(
            buf, mpv.handle, kwargs_to_render_param_array(kwargs)
        )
    ...
    def __setattr__(self, name, value):
        if name.startswith("_"):
            ...
            _mpv_render_context_set_update_callback(
                self._handle, self._update_fn_wrapper, None
            )
