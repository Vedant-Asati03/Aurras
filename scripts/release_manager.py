...
def safe_file_operation(operation_name: str, file_path: Path, operation_func):
    ...
    try:
        ...
        logger.error(f"❌ {operation_name} failed: {file_path} not found")
        return False
    except PermissionError:
        logger.error(f"❌ {operation_name} failed: Permission denied for {file_path}")
        return False
...
def fetch_pypi_sha256(package_name: str, version: str) -> str:
    ...
    try:
        ...
        logger.info(f"📦 Fetching SHA256 for {package_name} v{version}...")
        ...
        if e.code == 404:
            logger.warning(f"⚠️  Package version {version} not found on PyPI yet")
            logger.info(
                "   This is expected for new releases - package will be uploaded later"
            )
        else:
            logger.error(f"⚠️  HTTP error {e.code}: {e.reason}")
        return "SKIP"
    except urllib.error.URLError as e:
        logger.error(f"⚠️  Network error: {e.reason}")
        return "SKIP"
    except Exception as e:
        logger.error(f"⚠️  Unexpected error fetching SHA256: {e}")
...
def update_all_versions(new_version: str) -> None:
    ...
    if not validate_semantic_version(new_version):
        logger.error("❌ Version update aborted due to invalid format")
        sys.exit(1)

    current_version = get_current_version()

    if current_version == new_version:
        logger.info(f"🎯 Version is already {new_version}")
        return
    ...
    if aur_pkgbuild.exists():
        print("🔐 Updating AUR SHA256 hash...")
        ...
        if sha256_hash != "SKIP":
            ...
            if re.search(sha256_pattern, content):
                ...
                logger.info("✅ Updated AUR SHA256 hash")
            else:
                logger.warning("⚠️  Could not find SHA256 pattern in PKGBUILD")

    # Special handling for Homebrew SHA256 update
    homebrew_formula = Path("packaging/homebrew/aurras.rb")
    if homebrew_formula.exists():
        print("🍺 Updating Homebrew SHA256 hash...")
        sha256_hash = fetch_pypi_sha256("aurras", new_version)
        ...
        if sha256_hash != "SKIP":
            ...
            if sha256_pattern in content:
                ...
                logger.info("✅ Updated Homebrew SHA256 hash")
            else:
                logger.warning(
                    "⚠️  Could not find SHA256 placeholder in Homebrew formula"
                )
    ...
    logger.info(f"   🤖 Automation: {automation_level}")
    ...
    if is_prerelease(new_version):
        ...
        logger.info("🚀 Stable release notes:")
    ...
    if failed_files:
        logger.error("")
        logger.error("⚠️  Some files failed to update. Please review and fix manually.")
        sys.exit(1)
...
def check_sync_status() -> None:
    ...
    current_version = get_current_version()
    logger.info(f"🔍 Checking sync status for version {current_version}")
    logger.info("")
    ...
    logger.info("")
    logger.info("🔐 SHA256 Hash Status:")
    ...
    if aur_file.exists():
        ...
        if "sha256sums=('SKIP')" in content:
            logger.warning("   ⚠️  AUR: Using SKIP placeholder")
            all_synced = False
        else:
            logger.info("   ✅ AUR: Has SHA256 hash")
    ...
    if homebrew_file.exists():
        ...
        if "PLACEHOLDER_SHA256_HASH" in content:
            logger.warning("   ⚠️  Homebrew: Using placeholder SHA256")
            all_synced = False
        else:
            logger.info("   ✅ Homebrew: Has SHA256 hash")

    logger.info("")
    if all_synced:
        logger.info("🎉 All files are in sync!")
    else:
        logger.warning("⚠️  Some files are out of sync.")
        if missing_files:
